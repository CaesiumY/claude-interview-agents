...
**호출 전 준비 (어절 수 계산)**: 메인 루프가 초안의 **본문 어절 수**(공백 구분,
마크다운 제목 `#`·인용 `>`·표·`[확인 필요]` 태그 제외)를 Bash 등으로 계산해 프롬프트에 주입합니다.
reviewer는 Bash가 없어 스스로 셀 수 없으므로 이 참고값이 가독성 판정의 기준이 됩니다.
...
- 이력서 원문 한 줄: [원문 그대로]
- 어절 수 참고값: 이력서 한 줄 X어절 / 초안 본문 Y어절 (오케스트레이터 계산)
- JD 컨텍스트: portfolio/<slug>/jd-context.md
